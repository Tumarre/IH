...
:bulb: _Hint_: Start with getting the DOM nodes for each product row. Currently, we have two products; thus, we have two rows with the class `product`. Maybe using [getElementsByclass](https://developer.mozilla.org/en-US/docs/Web/API/Document/getElementsByclass) could serve well here.
